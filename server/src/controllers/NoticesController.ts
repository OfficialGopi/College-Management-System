...
export const postNotices = TryCatch(async (req, res, next) => {
    ...
    if (!title || !details) next(new ErrorHandler('Title and content are required', 400));
...
export const updateNotices = TryCatch(
    async (req, res, next) => {
        const { id } = req.params;
        const { title, details } = req.body;
        if (!title || !details) next(new ErrorHandler('Title and content are required', 400));
        const updatedNotice = await NoticeModel.findByIdAndUpdate(id, { title, details }, { new: true });
...
export const deleteOneNotice = TryCatch(
    async (req, res, next) => {
        const { id } = req.params;
...
export const deleteAllNotices = TryCatch(
    async (req, res, next) => {
